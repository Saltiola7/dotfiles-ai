...
| V3.20-1 | Add atomic multi-page captures and compact history transport | high | ready | V3.19-1 | Immutable logical manifests, aggregate projection, bounded drill-down, replay | Ledger and history filters | no | Cohorts exceed 100 and current output truncates | M | 201-session atomic save/replay, response-size, ordering, deletion, malformed-state, and latency tests |
...
| V3.22-1 | Add longitudinal benchmarks and implementation effects | high | ready | V3.20-1,V3.21-1 | Versioned benchmark runs, 30-day before/after windows, recurrence, association tracking | Captures, telemetry, merges, deployments, review runs | no | Implemented changes are not evaluated over time | M | Baseline, incomplete-window, replay, confounder, neutral, invalid, and regression tests |
