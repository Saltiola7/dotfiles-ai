...
| V3.20-1 | Add atomic multi-page captures and compact history transport | high | ready | V3.19-1 | Immutable logical manifests, helper-derived aggregate projection, bounded drill-down, replay | Ledger and history filters | no | Cohorts exceed 100 and current output truncates | M | 201-session contiguous atomic save/replay, gap/overlap/duplicate/projection-mismatch rejection, response-size, ordering, deletion, malformed-state, and latency tests |
