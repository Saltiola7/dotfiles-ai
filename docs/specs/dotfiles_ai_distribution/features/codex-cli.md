...
fail before replacement. The manifest is written last. `auth.json`, sessions,
logs, plugins, package metadata, and unrelated files are never inspected or
modified.
...
sequenceDiagram
    ...
    accDescr: Chezmoi validates configuration, installs or verifies the platform binary, stages managed files, rejects unsafe targets, atomically projects the ownership manifest last, and installs a PATH-preferred wrapper. A later explicit launch derives CODEX_HOME and executes the verified real binary without changing desktop state.
    ...
    P->>P: Validate home, sentinel, target, and collisions
    P-->>Z: Atomic projection; manifest last
...
validates state and collisions, and publishes the ownership manifest last.
Apply does not restart processes. A later explicit `codex` call reaches the
PATH-preferred wrapper, which derives `CODEX_HOME`, verifies the real binary, and
executes it as an argument vector.
