...
|---|---|---|---|---|---|---|---|---|---|---|
| DAI-017-F1 | Make Lima checks capability-aware and install required OpenCode in CI | high | in_progress | DAI-017 | External-binary test boundaries, pinned CI setup, and CI evidence | Existing deterministic rendering assertions and test workflow | yes | GitHub macOS runners omit operator binaries, blocking otherwise passing protected-delivery promotion | S | Full pytest matrix passes with an explicit Lima skip and required pinned OpenCode parser validation |
