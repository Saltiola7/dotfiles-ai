...
|---|---|---|---|---|---|---|---|---|---|---|
| DAI-017-F1 | Make optional Lima and OpenCode integration checks capability-aware in CI | high | in_progress | DAI-017 | Two external-binary test boundaries and CI evidence | Existing deterministic rendering assertions and test workflow | yes | GitHub macOS runners omit optional operator binaries, blocking otherwise passing protected-delivery promotion | S | Full pytest matrix passes with explicit skips when binaries are absent and full integration checks when present |
