...
|---|---|---|---|---|---|---|---|---|---|---|
| DAI-023 | Add an opt-in machine-local Lima state root without changing teammate defaults | high | implementing | - | Sandbox config, bounded instance lifecycle, controller environment, tests, and distribution lifecycle docs | Existing Lima workspace contracts | no | Storage migration and deployment share one sensitive runtime boundary | S | Focused rendering/controller tests, full affected suite, and live relocated VM checks |
