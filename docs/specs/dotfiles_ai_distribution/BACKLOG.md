...
|---|---|---|---|---|---|---|---|---|---|---|
| DAI-016-F1 | Repair deterministic R&D launch and prove the complete subprocess boundary | high | in_progress | DAI-016 | Session lookup, launch cleanup, E2E coverage, and one controlled live run | DAI-016 orchestration contract and 2026-07-30 failed dispatch evidence | no | Full plugin initialization returned empty session-list output before process start, while mocked launch tests could not exercise that boundary | S | Focused red/green E2E, affected QA, targeted deployment identity, Hermes run, and exact worker/session registration |
