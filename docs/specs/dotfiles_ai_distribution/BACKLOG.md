...
|---|---|---|---|---|---|---|---|---|---|---|
| DAI-021 | Run continuous isolated R&D lenses with review-session telemetry | high | in_progress | DAI-016-F3 | Per-lens scheduler, review-session attribution, autonomous readiness, Hermes fill loop, specs and operations | Federated history, improvement ledger, runner state, OpenCode command, Hermes supervisor | no | One global pass cannot exhaust lenses independently and lets every lens reconsider review sessions | L | Per-lens concurrency/backoff tests, attribution adapter tests, autonomous transition test, affected QA, live six-lens fill smoke |
