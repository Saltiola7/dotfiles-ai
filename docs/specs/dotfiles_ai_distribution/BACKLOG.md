...
|---|---|---|---|---|---|---|---|---|---|---|
| DAI-016-F1 | Repair deterministic R&D launch and prove the complete subprocess boundary | high | in_progress | DAI-016 | Session lookup, supervisor invocation, launch cleanup, host capture bound, E2E coverage, and one controlled live round | DAI-016 orchestration contract and 2026-07-30 failed dispatch evidence | no | Full plugin initialization returned empty session-list output, generated supervisor instructions omitted argparse ordering, mocked tests missed both boundaries, and live host capture exceeded the shared 120-second source bound | S | Focused red/green E2E, affected QA, targeted deployment identity, complete Hermes run, and exact worker/session/lens result |
