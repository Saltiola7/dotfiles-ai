...
reads. Given a user approves broader JQL research, they also allow direct bounded
work-item search. Other direct ACLI commands, absolute paths, common wrappers,
...
- ACLI permissions allow direct auth-status, work-item view, bounded search, and
  comment-list reads while denying other direct and common wrapped forms. Prompt
  contracts further restrict fields, limits, consent, and privacy.
...
| ACLI boundary | Direct read allowlist, Plan parity, wrapper/mutation denial, and bounded prompt contract | Writing and control-plane tests plus rendered config | Required when changed |
