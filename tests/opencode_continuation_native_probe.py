...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="continuation-native-", dir=args.scratch) as temporary:
        ...
        helper.write_text(f"#!/bin/sh\nexec {shlex.quote(sys.executable)} {shlex.quote(str(source / 'dot_local/bin/executable_dbsctrctl'))} \"$@\"\n")
        helper.chmod(0o700)
        ...
        for name in ("lib/dbsctr-runtime.ts", "lib/continuation.ts", "plugins/continuation.ts", "tools/dbsctr.ts"):
            ...
            shutil.copy2(source / "private_dot_config/opencode" / name, destination)
        if args.sdk_modules:
