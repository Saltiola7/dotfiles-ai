...
def test_dbsctr_safe_git_permissions_and_reviewer():
    ...
    assert bash["gh *"] == "ask"
    assert bash["gh issue list --state all --limit 100 --json number,title,state,labels"] == "allow"
    assert bash["gh pr list --state all --limit 100 --json number,title,state,isDraft,headRefName,baseRefName"] == "allow"
